...
#ifndef FAMGYANDROIDINJECT_TRACED_HPP
#define FAMGYANDROIDINJECT_TRACED_HPP
...
public:
    ...
    void *findFunction( const char* library, void* local_addr ){
        ...
        return (void *)( (uintptr_t)local_addr - (uintptr_t)local_handle + (uintptr_t)remote_handle);
    }
    ...
    unsigned long dlopenInjectSo( const char *libname ) {
    ...
    Traced( pid_t pid ) : _pid(pid) {
        if( trace( PTRACE_ATTACH ) != -1 ){
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "===Traced===", "Failed to attach to process %d.", _pid );
        }
    }
...
#endif //FAMGYANDROIDINJECT_TRACED_HPP

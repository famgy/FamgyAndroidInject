...
enum ele7en_status registerInlineHook(uint32_t target_addr, uint32_t new_addr, uint32_t **proto_addr)
{
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "===inlinehook===", "registerInlineHook start");
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "===inlinehook===", "registerInlineHook end");
...
{
	__android_log_print(ANDROID_LOG_DEBUG, "===inlinehook===", "inlineHook start");
	...
	__android_log_print(ANDROID_LOG_DEBUG, "===inlinehook===", "inlineHook end");
	...
	if (item->status == REGISTERED) {
		...
		__android_log_print(ANDROID_LOG_DEBUG, "===inlinehook===", "doInlineHook start");

		doInlineHook(item);

		__android_log_print(ANDROID_LOG_DEBUG, "===inlinehook===", "doInlineHook end");
